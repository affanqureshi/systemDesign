...
A network partition occurs when servers in a distributed system cannot communicate due to network failures,
even though the servers themselves are still running. This is a common problem in distributed systems and must be handled during system design.

According to the CAP Theorem, during a network partition a system can guarantee either Consistency (C) or Availability (A),
but not both, while always maintaining Partition Tolerance (P).
